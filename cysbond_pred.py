...
#antes de nada comprobamos que el fichero pasado es un PDB
if file.split('.')[-1].lower() != 'pdb':
    sys.exit("\nERROR: The entered file must be in PDB format.\nEnd of the program.")
...
if 'alphafold' in structure.header['name'].lower() or 'af-' in "".join(map(lambda el: str(el).lower(),list(structure.header.values()))) or 'predict' in "".join(map(lambda el: str(el).lower(),list(structure.header.values()))):
...
for cys1 in filtered_cys:
    ...
    for cys2 in filtered_cys:
        ...
        if distance_inf<=(SG_atom1 - SG_atom2)<=distance_sup and angle_inf<=dihedral_cys(cys1,cys2)<=angle_sup:
            ...
            info['Cys1'] = {'Name':cys1.get_resname(), 'Pos':cys1.id[1], 'Chain':cys1.get_parent().id}
            #de la segunda
            info['Cys2'] = {'Name':cys2.get_resname(), 'Pos':cys2.id[1], 'Chain':cys2.get_parent().id}
...
for bond in unique_bonds:
    ...
    pos1 = bond['Cys1']['Pos']
    pos2 = bond['Cys2']['Pos']
    chain1 = bond['Cys1']['Chain']
    chain2 = bond['Cys2']['Chain']
    ...
    for position, chain in zip([pos1,pos2],[chain1,chain2]):
        script_lines.append(f"cmd.select('SG{position}', 'chain {chain} and resi {position} and name SG')")
        script_lines.append(f"cmd.select('CB{position}', 'chain {chain} and resi {position} and name CB')")
        script_lines.append(f"cmd.select('CA{position}', 'chain {chain} and resi {position} and name CA')")
    ...
    script_lines.append(f"cmd.label('c. {chain1} and n. SG and i. {pos1}', '\"SG-CYS{pos1}-{chain1}\"')")
    script_lines.append(f"cmd.label('c. {chain2} and n. SG and i. {pos2}', '\"SG-CYS{pos2}-{chain2}\"')")
